...
class Edit {
	...
	public static function updateContent($cid, $content) {
		$DBH = new myPDO();
	...
	public static function addContent($vfsid, $dest, $blockID) {
		$DBH = new myPDO();
	...
	public static function removeContent($remID) {
		$DBH = new myPDO();
		
		$DBH->prepare("DELETE FROM `web_content` WHERE `contentID`= ? LIMIT 1;")->execute( $remID );
		
		if ($DBH->errorCode()!=0) return 'noCheck';
		
		// the following will be replaced with on delete cascate table relation - TODO: need to only set the state as deleted
		$DBH->prepare("DELETE FROM `webConHistory` WHERE `contentID`=? LIMIT ?;")->execute( $remID, self::$NUM_OF_HISTORY );
